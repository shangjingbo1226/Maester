...
from score import *
from NERFeatureGenerator import *
import operator
import matplotlib
matplotlib.use('Agg')
...
def train():
    ...
    w = np.array([1 if y == 0 else 3 for y in data_y])
    print 'w:'
    print w
    print np.mean(w)
    ...
    plot_dict['Tfidf'] = 0
    plot_dict['Svd'] = 0
    plot_dict['Word2Vec'] = 0
    plot_dict['NamedEntity'] = 0

    for key in importance:
        if int(key[1:]) < 42:
            plot_dict['Tfidf'] = plot_dict['Tfidf'] + (importance[key] / 42.0)
        elif int(key[1:]) < 143:
            plot_dict['Svd'] = plot_dict['Svd'] + (importance[key] / 101.0)
        elif int(key[1:]) < 744:
            plot_dict['Word2Vec'] = plot_dict['Word2Vec'] + (importance[key] / 601.0)
        elif int(key[1:]) < 748:
            plot_dict['NamedEntity'] = plot_dict['NamedEntity'] + (importance[key] / 4.0)
    summation = sum(plot_dict.values())
        # plot_dict['TfidfFeature'] + plot_dict['SvdFeature'] + plot_dict['Word2VecFeature'] + plot_dict['NamedEntityFeature']
    #for key in plot_dict:
        #plot_dict[key] = plot_dict[key] / float(summation)
    importance = plot_dict

    importance = sorted(importance.items(), key=operator.itemgetter(1), reverse=True)

    y = [k[0] for k in importance]
    x = [k[1] for k in importance]
    df = pd.DataFrame(importance, columns=['feature', 'fscore'])
    plt.figure()
    y_pos = np.arange(len(y))
    plt.bar(y_pos, x, align='center', alpha=0.5)
    plt.xticks(y_pos, y)
    plt.title('Importance for Each Feature')
    plt.ylabel('Feature Importance')
    print importance
    # plt.savefig('features_importance.png')
    # df.plot()
    # df.plot(x='feature', y='fscore', legend=False, figsize=(10, 25))
    plt.gcf().savefig('features_importance.pdf')
    ...
    # df.plot()
    # df.plot(x='feature', y='fscore', legend=False, figsize=(10, 25))
    plt.gcf().savefig('Relative_features_importance.pdf')

    print 'passed train'
    pickle.dump(bst, open("tree_model.dat", "wb"))
    ...
    pred_prob_y = bst.predict(dtest).reshape(test_x.shape[0], 1)
    # pred_y = np.argmax(pred_prob_y, axis=1)
    pred_y = pred_prob_y > 0.5
    print 'pred_y.shape:'
    print pred_y.shape
    predicted = [LABELS_RELATED[int(a)] for a in pred_y]
